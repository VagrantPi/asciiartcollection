...
BASE_URL="https://raw.githubusercontent.com/VagrantPi/asciiartcollection/refs/heads/master"

WORKING_DIR="$HOME/.asciiarting"
THEME_LIST="$WORKING_DIR/list.txt"
THEME_FOLD="$WORKING_DIR/theme"

## 判斷本地 clone 還是 remote 執行
SCRIPT_DIR="$(cd "$(dirname "$0")" 2>/dev/null && pwd)"
if [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/../json/list.json" ]; then
...
## 取得清單（本地優先，fallback 到 remote）
if [ -n "$JSON_SRC" ] && [ -f "$JSON_SRC" ]; then
    list=$(cat "$JSON_SRC" | tr ',' '\n' | sed -n 's/.*\["\([^"]*\)"\].*/\1/p')
else
    list=$(curl -fsSL "$BASE_URL/json/list.json" | tr ',' '\n' | sed -n 's/.*\["\([^"]*\)"\].*/\1/p')
fi
...
while [ "$input_theme" == "false" ]; do
    ...
    if [[ "$theme_num" =~ ^[0-9]+$ ]]; then
        ...
        if theme=$(showFileSpLine "$THEME_LIST" "$theme_num"); then
            ## 嘗試取得主題檔案（本地優先，fallback 到 remote）
            if { [ -n "$THEME_SRC" ] && cp "${THEME_SRC}${theme}.txt" "$THEME_FOLD/${theme}.txt" 2>/dev/null; } || \
               curl -fsSL "$BASE_URL/media/${theme}.txt" -o "$THEME_FOLD/${theme}.txt" 2>/dev/null; then
                clear
...
## 複製安裝腳本（本地優先，fallback 到 remote）
if [ -n "$INSTALLER_SRC" ] && [ -f "$INSTALLER_SRC" ]; then
    cp "$INSTALLER_SRC" "$WORKING_DIR/ASCIIArtInstaller.sh"
else
    curl -fsSL "$BASE_URL/scripts/ASCIIArtInstaller.sh" -o "$WORKING_DIR/ASCIIArtInstaller.sh"
fi
...
## 詢問完成
echo "稍後會使用 ${EDITOR:-vi} 編輯motd，你可以加入你需要的內容、或是刪掉內容。"
read -p "按下Enter開始"
